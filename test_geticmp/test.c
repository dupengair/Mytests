...
int main(int argc, char* argv[])
{
    ...
    unsigned long  size = MAX_SIZE, num=0, i;
    int sock = -1;
    ...
    while(quit_flag == 0)
        {
        num = sendto(sock, buf, sizeof(buf), 0, (struct sockaddr*)&addr_to, sizeof(addr_to));
        printf("send %d \n", num);
        if(getsockopt(sock, SOL_SOCKET, SO_ERROR, &optval, &len) > 0)
            {
                printf("get error: optval = %d, len = %d\n", optval, len);
            }
        else
            {
            perror("getsockopt");
